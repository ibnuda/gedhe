...
almost_flat_sphere()
{
    resize([ 15, 15, 4.5 ]) translate([ 0, 0, -90 ]) sphere(r = 90, $fn = 60);
}
...
tenting_cylinder()
{
    hull()
    {
        translate([ 0, 0, 20 ]) almost_flat_sphere();
...
base_shape()
{
    difference()
    {
        ...
        translate([0, 0, 10]) linear_extrude(height = 17) alpha_holes();
        translate([0, 0, 10]) linear_extrude(height = 17) thumb_holes();
        translate([0, 0, 15]) linear_extrude(height = 17) alpha_holes(19);
        translate([0, 0, 15]) linear_extrude(height = 17) thumb_holes(19);
    }
}
